...
treasnames <- paste0(unname(treas), ' (', names(treas),  ')')
...
#    scale_y_log10() + #limits = c(10, NA)) +
    ...
    ggtitle('Stock and crypto market data')
...
## In substitution, need to substitute on the full expression, else "DGS1" and "DGS10" get confused
for (i in 1:length(treas))
    rates$series_id <- gsub(paste0('^',treas[i], '$'), treasnames[i], rates$series_id)
rateorder <- treasnames #c('DGS3MO','DGS1','DGS10','DGS30')
rates$series_id <- factor(rates$series_id, levels = treasnames)
